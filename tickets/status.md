...
- Perspective overhaul (PERSP-000..PERSP-010) intentionally re-opens `CourtProjection` and `CourtLayoutSystem`. While these tickets are open, that Phase 5 prohibition does **not** apply to them — but the overhaul still must not change `Court.*` court constants, `BallState`/`PlayerState` semantics, gameplay rules, scoring, AI behavior, the locked control contract, or add new dependencies. The 3/4 perspective product rule from `docs/build-spec.md` §2 still holds: no pure side-view, no pure top-down. Read `tickets/perspective-overhaul-000-overview.md` before starting any PERSP ticket.
